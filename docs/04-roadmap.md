...
El **port a Lynx está funcional** y vive en la raíz del repositorio: 14 pantallas, 5 pestañas,
sistema de diseño v2, capa de datos con repos y una suite de tests propia. Lo que falta no es UI: son
las piezas que exigen **host nativo** y **persistencia durable**. El plan por fases completo está en
...
| **Host nativo** | App Android propia con `LynxView` y módulo nativo SQLite | Tickets 1 y 2 del [`001`](../specs/001-host-nativo-y-almacenamiento-durable.md) |
| **Reestructura del repo** | Lynx promovido a la raíz; la app anterior y sus dependencias, fuera | [`004`](../specs/004-reestructura-del-repo-y-retirada-de-expo.md) |

## 🚧 Próximo bloque (specs aprobados)

El orden no es negociable en un punto: **el almacenamiento durable antes que todo** (registrar sin
persistir es teatro).

| Spec | Qué resuelve | Bloquea |
|------|--------------|---------|
| [`001`](../specs/001-host-nativo-y-almacenamiento-durable.md) | Adaptador durable sobre el módulo SQLite del host | 002, 003 |
...
Lo que aún no está migrado desde la app anterior, con su ruta en la etiqueta de respaldo, está en
[13-pendiente-de-migrar-desde-expo](./13-pendiente-de-migrar-desde-expo.md).
...
- [x] Compartir resultado (pendiente de la captura nativa).
